...
#include "hevc.h"  //Modify for HEVC-FLV Support
...
static const AVCodecTag flv_video_codec_ids[] = {
    ...
    { AV_CODEC_ID_HEVC,     FLV_CODECID_HEVC },  //Modify for HEVC-FLV Support
    { AV_CODEC_ID_NONE,     0 }
};
...
static void flv_write_codec_header(AVFormatContext* s, AVCodecParameters* par, int64_t ts) {
    ...
    if (par->codec_id == AV_CODEC_ID_AAC || par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_HEVC  //Modify for HEVC-FLV Support
        ...
        if (par->codec_id == AV_CODEC_ID_AAC) {
           ...
           if (par->codec_id == AV_CODEC_ID_HEVC) {  //Modify for HEVC-FLV Support
    ...
    if (flv->flags & FLV_NO_SEQUENCE_END) {
        ...
        for (i = 0; i < s->nb_streams; i++) {
            ...
            if (par->codec_type == AVMEDIA_TYPE_VIDEO &&
                    (par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_HEVC || par->codec_id == AV_CODEC_ID_MPEG4))  //Modify for HEVC-FLV Support
...
static int flv_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (par->codec_id == AV_CODEC_ID_VP6F || par->codec_id == AV_CODEC_ID_VP6A ||
        ...
        flags_size = 2;
    else if (par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_HEVC || par->codec_id == AV_CODEC_ID_MPEG4)  //Modify for HEVC-FLV Support
        flags_size = 5;
    else
        flags_size = 1;
    ...
    if (par->codec_id == AV_CODEC_ID_H264 || par->codec_id == AV_CODEC_ID_MPEG4) {
        ...
        if (par->extradata_size > 0 && *(uint8_t*)par->extradata != 1)
            if ((ret = ff_avc_parse_nal_units_buf(pkt->data, &data, &size)) < 0)
                return ret;
    } else if (par->codec_id == AV_CODEC_ID_HEVC) {  //Modify for HEVC-FLV Support
        if (par->extradata_size > 0 && *(uint8_t*)par->extradata != 1)
